...
jQuery(function($) {
	...
	function enableDemoAjax() {		
		...
		var demo_ajax_options = {
			 ...
			 'default_url': 'fw/ace2/html/ajax/content/index',//default hash
			 'content_url': function(hash) {
				return _contextPath + "/" + hash;				
				//***NOTE***
				...
				if(path.match(/(\/ajax\/)(index\.html)?/)){
					alert(path.replace(/(\/ajax\/)(index\.html)?/, '/ajax/content/'+hash.replace(/^page\//, '')+'.html'));
